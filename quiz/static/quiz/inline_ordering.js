/* Vendored this from https://github.com/centralniak/django-inline-ordering */
var ORDER_FIELD_NAME = 'order';
var InlineOrdering = {
    ...
    getOrderables: function () {
        ...
        for (i = 0; i < allInlineRows.length; i = i + 1) {
            if (InlineOrdering.jQuery('.'+ ORDER_FIELD_NAME +' input, .field-'+ ORDER_FIELD_NAME +' input', allInlineRows[i]).val()) {
    ...
    init: function (jQuery) {
        ...
        InlineOrdering.jQuery('div.'+ ORDER_FIELD_NAME).hide();
        InlineOrdering.jQuery('td.'+ ORDER_FIELD_NAME +' input').hide();
    ...
    update: function () {
        InlineOrdering.getOrderables().each(function (i) {
            InlineOrdering.jQuery('input[id$='+ORDER_FIELD_NAME+']', this).val(i + 1);
